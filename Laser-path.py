...
def find_point(x1, y1, x2, y2, x3, y3, x4, y4):
    # точка пересечения отрезков + расстояние до неё
    coord = [0] * 3
...
def find_mir(Cx, Cy, Ax, Ay, A1x, A1y, A2x, A2y):
    # находим точку, симметричную нормали зеркала
    # for point C(laser: C, A-C) and wall (A1,A2),D=A2-A1,
    # A between A1-A2; B normal, new point C+2fD,
    # f=(Ay-Cy+(Cx-Ax)By/Bx)/(Dy-ByDx/Bx)
    # new way (A, C+2fD-A)
    coord = [0] * 2
    Dx = A2x - A1x
    Dy = A2y - A1y
...
def Laser_step(r, l1):
    # получаем координаты стен и лазера, возвращаем новые координаты лазера
    l2 = Laser
    mas1 = [0] * 3
    mas2 = [0] * 2
